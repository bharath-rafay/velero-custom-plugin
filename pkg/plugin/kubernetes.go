...
const (
	...
	// VeleroDeploymentName   = "velero-7w2lnkp-nginxt"
...
func getDeployment(clientset kubernetes.Interface, namespace string, opts *localVolumeObjectStoreOpts) (*appsv1.Deployment, error) {
	existingDeployments, err := clientset.AppsV1().Deployments(namespace).List(context.TODO(), metav1.ListOptions{})
	...
	for _, v := range existingDeployments.Items {
		if strings.Contains(v.Name, "velero") {
			return &v, nil
		}
	}
	return nil, fmt.Errorf("no deployment velero found")
}
...
func getPluginConfigMap(kind VolumeType) (*corev1.ConfigMap, error) {
	listOpts := metav1.ListOptions{
		LabelSelector: fmt.Sprintf("neridio.io/%s=%s", string(kind), veleroplugin.PluginKindObjectStore),
	}
